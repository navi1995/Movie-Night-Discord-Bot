...
module.exports = {
	...
	async execute(interaction, main, settings) {
		...
		if (settings.addMoviesRole && !interaction.member.roles.cache.has(settings.addMoviesRole) && !interaction.member.permissions.has("ADMINISTRATOR")) {
			return interaction.editReply("Non-administrators can only add movies if they have specified role in addMoviesRole has cleared or a specific role by an administrator.");
		}
				...
				if (newMovie) {
					return newMovie.save((err) => {
						if (err && err.name == "MongoServerError") {
							return interaction.editReply("Movie already exists in the list. It may be marked as 'Viewed'");
						}

						if (err) {
							console.log(err);
							return interaction.editReply("Something went wrong, couldn't run command");
						}
						...
						if (data && (data.total_results > 1 || (data.movie_results && data.movie_results.length > 1))) {
							...
							return interaction.editReply({ embeds: [movieEmbed] }).then(async () => {
							...
							return interaction.editReply({ embeds: [main.buildSingleMovieEmbed(newMovie, "Movie Added!")] });
						}
					});
				}
			});
