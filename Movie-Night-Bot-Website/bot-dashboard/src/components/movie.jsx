...
export function MovieComponent(props) {
	...
	return (
		<div className="movie" key={movie.imdbID} style={{ padding: 0 }}>
			...
			{adminLevel === "admin" && (
				<ButtonGroup className="movie-button-group">
					<Button variant="primary" onClick={toggleViewed}>
						{"Set " + (movie.viewed ? "Unviewed" : "Viewed")}
					</Button>
			...
			<div className="movie-details">
				<h3>
					{movie.imdbID && (
						<a target="_blank" rel="noopener noreferrer" href={`https://www.imdb.com/title/${movie.imdbID}`}>
							{movie.name}
						</a>
					)}
					{!movie.imdbID ? movie.name : ""}
				</h3>
			...
			<div className="button-container">
				<Button onClick={toggleTextBody} variant="primary" className="w-100">
					<FontAwesomeIcon icon={!show ? faPlus : faMinus} />
				</Button>
			</div>
		</div>
	);
