module.exports = {
	...
	async execute(message, args, main) {
		...
		if (!message.guild.roles.resolve(pollRole) && pollRole !== "clear") {
			return message.channel.send("Please provide a valid role you'd like to set in the format `pollrole [roleName]`, or to clear settings use `pollrole clear`");
		} else {
			...
			return main.setting.updateOne({ guildID: message.guild.id }, { pollRole }, err => {
				if (!err) {
					return message.channel.send(pollRole ? `Users with administrator or the role <@&${pollRole}> will now be able to run polls.` : "Setting for role allowed to run polls has been cleared. Only admins may run polls now.");
				} else {
